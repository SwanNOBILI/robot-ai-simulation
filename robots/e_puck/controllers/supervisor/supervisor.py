...
PI = np.pi
HEADLESS = (os.environ.get("MODE", "train") == "train")
...
# Boxes parameters
minimal_box_size = 0.033*1.1    # taller than the height of the Robot sensors (0.033 = height of the Robot sensors)
maximal_box_size = minimal_box_size*3
min_boxes = 5
max_boxes = 3*min_boxes




# Get a random position in the Arena (+/- margin to avoid boxes to spawn in the Arena walls)
...
# Get a random start position and goal position
...
def generate_non_overlapping_position_near_path(existing_boxes, new_box_size, max_attempts, 
    ...
    for _ in range(max_attempts):
        ...
        for ex, ey, esize in existing_boxes:
            ...
            min_dist_x = (sx_new + sx_i) / 2 + maximal_box_size*1.2 # min_dist_x between two boxes > maximal_box_size*1.2
            min_dist_y = (sy_new + sy_i) / 2 + maximal_box_size*1.2 # min_dist_y between two boxes > maximal_box_size*1.2
...
# E-puck spawn
def e_puck_spawn(start_position, goal_position):
...
# Boxes spawn
def boxes_spawn(start_position, goal_position, box_spawn_attempts):
    positions = [(start_position[0], start_position[1], [0, 0, 0]), 
                 (goal_position[0], goal_position[1], [0, 0, 0])]
...
if HEADLESS:
    ...
    episode = 0
    initial_children_count = children.getCount()
    for episode in range(total_episodes):
        # World Initalization
        start_position, goal_position = get_start_and_goal_position(0.3*np.linalg.norm(arena_size), maximal_box_size)
        e_puck_spawn(start_position, goal_position); boxes_spawn(start_position, goal_position, 1000)
        # Supervisor Loop
        while supervisor.step(TIMESTEP) != -1:
            if receiver.getQueueLength() > 0:
                ...
                receiver.nextPacket()
        # Ending episode
        while children.getCount() > initial_children_count: children.removeMF(-1)

# If we are in EVAL MODE
else:
    # World Initalization
    start_position, goal_position = get_start_and_goal_position(0.3*np.linalg.norm(arena_size), maximal_box_size)
    e_puck_spawn(start_position, goal_position); boxes_spawn(start_position, goal_position, 1000)
